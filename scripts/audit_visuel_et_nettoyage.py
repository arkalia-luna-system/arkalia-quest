...
class VisualAuditor:
    ...
    def find_unused_files(self):
        ...
        for root, dirs, files in os.walk(PROJECT_ROOT):
            dirs[:] = [d for d in dirs if d not in ['venv', '__pycache__', '.git', 'node_modules', 'build', 'dist']]
            for file in files:
                if file.endswith('.py'):
    ...
    def find_orphaned_assets(self):
        ...
        for root, dirs, files in os.walk(PROJECT_ROOT / "templates"):
            for file in files:
                if file.endswith('.html'):
                    try:
                        ...
                        img_refs = re.findall(r'src=["\']([^"\']*\.(?:png|jpg|jpeg|gif|svg|ico))["\']', content)
            ...
            for file in files:
                if file.endswith('.py'):
                    try:
                        ...
                        # Extraire les références aux assets
                        asset_refs = re.findall(r'["\']([^"\']*\.(?:css|js|png|jpg|jpeg|gif|svg|ico))["\']', content)
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.match(r'^\.[a-zA-Z][a-zA-Z0-9_-]*\s*\{', line):
                        class_name = re.match(r'^\.([a-zA-Z][a-zA-Z0-9_-]*)\s*\{', line).group(1)
        ...
        for html_file in html_files:
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if re.search(r'<(center|font|marquee|blink|applet|embed|object)\b', line, re.IGNORECASE):
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "problemes_visuels": issue_counts.get("problemes_css", 0) + issue_counts.get("problemes_html", 0) + issue_counts.get("problemes_js", 0)
            }
        }
    ...
    def _generate_markdown_report(self, report: Dict[str, Any]):
        ...
        with open(REPORT_MD, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **Fichiers analysés:** {report['stats']['fichiers_analyses']}\n")
            ...
            f.write(f"- **Fichiers inutilisés:** {report['summary']['fichiers_inutilises']}\n")
            f.write(f"- **Assets orphelins:** {report['summary']['assets_orphelins']}\n")
            f.write(f"- **Problèmes visuels:** {report['summary']['problemes_visuels']}\n")
                ...
                if issues:
                    ...
                    for issue in issues[:10]:  # Limiter à 10 par catégorie
                        if isinstance(issue, dict):
                            f.write(f"- **{issue.get('fichier', 'N/A')}:** {issue.get('probleme', str(issue))}\n")
                        else:
                            f.write(f"- {issue}\n")
                    if len(issues) > 10:
