...
class AccessibilityTester:
    ...
    def test_focus_management(self):
        ...
        print(f"✅ Gestion du focus: {focus_indicators}/{total_expected} éléments trouvés")
        return score
    ...
    def test_semantic_html(self):
        ...
        semantic_tags = [
            ...
            'ul', 'ol', 'li', 'p', 'blockquote', 'figure', 'figcaption'
        ]
        ...
        print(f"✅ HTML sémantique: {semantic_elements}/{total_expected} éléments trouvés")
        return score
    ...
    def test_screen_reader_support(self):
        ...
        for pattern in sr_patterns:
            if soup.find(string=re.compile(pattern, re.IGNORECASE)) or soup.find(attrs=re.compile(pattern, re.IGNORECASE)):
                sr_elements += 1
        ...
        print(f"✅ Support lecteurs d'écran: {sr_elements}/{total_expected} éléments trouvés")
        return score
    ...
    def test_responsive_accessibility(self):
        ...
        print(f"✅ Accessibilité responsive: {responsive_elements}/{total_expected} éléments trouvés")
        return score
    ...
    def generate_recommendations(self):
        ...
        for test_name, test_result in self.results["tests"].items():
            if test_result["score"] < 80:
                recommendations.append(f"Améliorer {test_name}: {test_result['score']:.1f}%")
    ...
    def display_report(self):
        ...
        print(f"❌ Problèmes critiques: {sum(1 for test in self.results['tests'].values() if test['score'] < 50)}")

        print("\n📋 Détail des tests:")
        for test_name, test_result in self.results["tests"].items():
            status = "✅" if test_result["score"] >= 80 else "❌"
            print(f"  {status} {test_name.replace('_', ' ').title()}: {test_result['score']:.1f}/100 - {test_result['found']}/{test_result['expected']} éléments trouvés")
