...
- **Cohérence globale** : Navbar unifiée (composant `navbar.html`) sur toutes les pages avec `active_page` et `profil` ; nav simplifiée (Accueil, Terminal, Tutoriel) jusqu’à acte_1 complété. Routes leaderboard et skill-tree passent `profil` pour cohérence. Tutoriel et skill-tree : skip-link + accessibilité.
