...
class ArkaliaLogger:
    ...
    def _log(
        ...
        *,
        ...
        extra.update({"timestamp": datetime.now(timezone.utc).isoformat(), "module_name": self.name})
...
def export_logs_to_json(output_file: str | None = None):
    ...
    if output_file is None:
        output_file = str(LOGS_DIR / f"logs_export_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json")
