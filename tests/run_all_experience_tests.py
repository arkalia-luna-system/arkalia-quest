...
class ExperienceTestRunner:
    ...
    def run_all_tests(self):
        ...
        self.results["overall_score"] = total_score / len(self.results["tests_run"]) if self.results["tests_run"] else 0
        
        # Génération du résumé
        self.results["summary"] = {
            "total_tests": len(self.results["tests_run"]),
            "successful_tests": successful_tests,
            "success_rate": (successful_tests / len(self.results["tests_run"]) * 100) if self.results["tests_run"] else 0,
    ...
    def _generate_recommendations(self):
        ...
        for test_name, score in test_scores.items():
            if score < 40:
                recommendations.append(f"❌ {test_name}: Nécessite une amélioration majeure (score: {score:.1f}/100)")
            elif score < 60:
                recommendations.append(f"⚠️  {test_name}: Amélioration recommandée (score: {score:.1f}/100)")
            elif score < 80:
                recommendations.append(f"👍 {test_name}: Bon, peut être optimisé (score: {score:.1f}/100)")
            else:
