...
class AdaptiveStorytelling:
    ...
    def record_player_choice(self, player_id: str, story_arc: str, choice: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if len(self.player_choices[player_id]["choices"]) > self.max_choices_history:
            self.player_choices[player_id]["choices"] = self.player_choices[player_id]["choices"][-self.max_choices_history:]
    ...
    def add_journal_entry(self, player_id: str, choice_record: Dict[str, Any]):
        ...
        if len(self.player_journals[player_id]["entries"]) > 500:
            self.player_journals[player_id]["entries"] = self.player_journals[player_id]["entries"][-500:]
    ...
    def check_easter_egg_conditions(self, player_id: str, egg: Dict[str, Any], choices: List[Dict[str, Any]]) -> bool:
    ...
    def evaluate_condition(self, player_id: str, condition: str, choices: List[Dict[str, Any]]) -> bool:
        """Évalue une condition d'Easter egg"""
        if condition == "luna_mentioned_10_times":
            luna_mentions = sum(1 for choice in choices if "luna" in choice.get("choice", "").lower())
            return luna_mentions >= 10
        
        elif condition == "deep_analysis":
            deep_choices = sum(1 for choice in choices if "deep" in choice.get("choice", "").lower() or "analyze" in choice.get("choice", "").lower())
            ...
            quantum_choices = sum(1 for choice in choices if "quantum" in choice.get("choice", "").lower())
    ...
    def trigger_easter_egg(self, player_id: str, egg_id: str):
        ...
        if player_id not in self.player_choices:
            self.player_choices[player_id] = {"choices": [], "unlocked_arcs": [], "discovered_easter_eggs": [], "lore_fragments": [], "story_progress": {}}
        
        self.player_choices[player_id]["discovered_easter_eggs"].append(egg_id)
        
        # Ajouter au journal
        if player_id not in self.player_journals:
            self.player_journals[player_id] = {"entries": [], "discoveries": [], "theories": []}
    ...
    def create_contextual_dialogue(self, context: str, playstyle: Dict[str, Any], choices: List[Dict[str, Any]]) -> str:
