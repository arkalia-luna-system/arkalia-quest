...
class TestGetState:
    ...
    def test_contains_expected_fields(self, client):
        r = client.get("/api/story/state")
        data = r.get_json()
        for field in ["scene_id", "chapter_id", "dialogue", "choices", "luna_trust", "xp"]:
    ...
    def test_cookie_persists_state(self, client):
    ...
    def test_scene_progress_present(self, client):
        r = client.get("/api/story/state")
        data = r.get_json()
...
class TestApplyChoice:
    def test_valid_choice(self, client):
        r = client.post("/api/story/choice", json={"scene_id": "s0_0", "choice_id": "c0_0_a"})
        data = r.get_json()
        assert data["success"] is True
        assert "choice_result" in data
        assert "next_state" in data

    def test_missing_scene_id_returns_400(self, client):
        r = client.post("/api/story/choice", json={"choice_id": "c0_0_a"})
        assert r.status_code == 400

    def test_missing_choice_id_returns_400(self, client):
        r = client.post("/api/story/choice", json={"scene_id": "s0_0"})
        assert r.status_code == 400

    def test_invalid_scene_returns_400(self, client):
        r = client.post("/api/story/choice", json={"scene_id": "inexistante", "choice_id": "c0_0_a"})
        assert r.status_code == 400

    def test_invalid_choice_returns_400(self, client):
        r = client.post("/api/story/choice", json={"scene_id": "s0_0", "choice_id": "inexistant"})
        assert r.status_code == 400

    def test_xp_in_result(self, client):
        r = client.post("/api/story/choice", json={"scene_id": "s0_0", "choice_id": "c0_0_a"})
        data = r.get_json()
...
class TestSetName:
    ...
    def test_name_persisted_in_state(self, client):
    ...
    def test_name_trimmed_to_30_chars(self, client):
        long_name = "A" * 50
...
class TestGetSummary:
    ...
    def test_exists_field(self, client):
        r = client.get("/api/story/summary")
        data = r.get_json()
        assert "exists" in data

    def test_flags_field_present(self, client):
        r = client.get("/api/story/summary")
        data = r.get_json()
...
class TestGetJournal:
    ...
    def test_moments_is_list(self, client):
        r = client.get("/api/story/journal")
        data = r.get_json()
...
class TestReset:
    ...
    def test_reset_restores_initial_state(self, client):
        client.post("/api/story/choice", json={"scene_id": "s0_0", "choice_id": "c0_0_a"})
...
class TestPages:
    ...
    def test_profil_page(self, client):
        r = client.get("/profil")
        assert r.status_code == 200
